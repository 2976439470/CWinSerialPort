...
BOOL CWinSerialPort::OpenPort(HWND pPortOwner,	// the owner (CWnd) of the port (receives message)
...
{
	if(InitPort( pPortOwner,	// the owner (CWnd) of the port (receives message)
		  portnr,		// portnumber (1..4)
		  baud,			// baudrate
		  parity,		// parity 
		  databits,		// databits 
		  stopbits,		// stopbits 
		  ...
		  readbuffersize,// size to the readbuffer
...
BOOL CWinSerialPort::InitPort(HWND pPortOwner,	// the owner (CWnd) of the port (receives message)
						   UINT  portnr,		// portnumber (1..4)
						   UINT  baud,			// baudrate
						   char  parity,		// parity 
						   UINT  databits,		// databits 
...
//	assert(pPortOwner != NULL);
	...
	m_nReadBufferSize = readbuffersize;
...
//
DWORD WINAPI CWinSerialPort::CommThread(LPVOID pParam)
{
	// Cast the void pointer passed to the thread back to
	// a pointer of CWinSerialPort class
	CWinSerialPort *port = (CWinSerialPort*)pParam;
...
// start comm watching
//
BOOL CWinSerialPort::StartMonitoring()
{
...
//
BOOL CWinSerialPort::RestartMonitoring()
{
...
//
BOOL CWinSerialPort::StopMonitoring()
{
...
//
void CWinSerialPort::ProcessErrorMessage(char* ErrorText)
{
...
// Write a character.
//
void CWinSerialPort::WriteChar(CWinSerialPort* port)
{
	BOOL bWrite = TRUE;
	BOOL bResult = TRUE;
...
//
void CWinSerialPort::ReceiveChar(CWinSerialPort* port)
{
	BOOL  bRead = TRUE; 
	BOOL  bResult = TRUE;
	DWORD dwError = 0;
...
//
void CWinSerialPort::WriteToPort(char* string)
{		
	assert(m_hComm != 0);
...
//
DWORD CWinSerialPort::GetCommEvents()
{
	return m_dwCommEvents;
}

DWORD CWinSerialPort::GetWriteBufferSize()
{
	return m_nWriteBufferSize;
}

DWORD CWinSerialPort::GetReadBufferSize()
{
...
void CWinSerialPort::WriteToPort(char* string,int n)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, sizeof(m_szWriteBuffer));
...
void CWinSerialPort::WriteToPort(LPCTSTR string)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, sizeof(m_szWriteBuffer));
	strcpy(m_szWriteBuffer, string);
...
void CWinSerialPort::WriteToPort(BYTE* Buffer, int n)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, sizeof(m_szWriteBuffer));
	int i;
...
void CWinSerialPort::SendData(LPCTSTR lpszData, const int nLength)
{
	assert(m_hComm != 0);
...
BOOL CWinSerialPort::RecvData(LPTSTR lpszData, int &nSize)
{
	//
	//接收数据
	//
	assert(m_hComm!=0);
	strncpy(lpszData,m_szReadBuffer,m_nReadSize);
	nSize = m_nReadSize;
	lpszData[nSize] = 0;
...
/*
BOOL CWinSerialPort::RecvData(LPTSTR lpszData, const int nSize)
{
	//
	//接收数据
	//
